...
- コンポーネントツリー内のリクエストが互いに依存している場合、リクエストがメモ化されていなければデータ取得は逐次的に行われる
	- あるデータ取得が他のデータ取得の結果に依存している場合は有効
	- ルートセグメントに対して `loading.tsx`を、ネストされたコンポーネントに対して `<Suspense>`を使用することで即座にローディング状態を表示できる
		- データリクエストによってルート全体がブロックされることを防ぎ、準備が整ったページの一部をユーザーがすぐに操作できるようになる
- デフォルトではレイアウトやページセグメントは並列でレンダリングされるため、リクエストを並列に開始される
	- `async/await`の性質上、同じセグメントまたはコンポーネント内で `await`されたリクエストがあると、それより下のリクエストはブロックされる
- データを並列で取得するには、データを使用するコンポーネントの外でリクエストを先に開始しておくことで、効率的に時間を節約できる
	- `Promise.all`を使うことですべてのリクエストが同時に始まり、全体の待ち時間を短縮できるがすべての `Promise`が解決されるまでユーザーには描画結果が表示されない
- 逐次的な遅延（ウォーターフォール）を防ぐもう一つの方法はpreloadパターンを使うこと
	- ブロッキングリクエストの前にユーティリティ関数を使ってリクエストを積極的に発行する方法
	- preloadという名前はあくまでもパターン名であり、APIではないので関数名は任意
- Reactの `cache`関数、preloadパターン、 `server-only`を組み合わせることで、アプリ全体で使用できるデータ取得関数を作成できる
	- このアプローチを使うことで、以下のことが可能になる
		- データの積極的取得（eager fetch）
		- レスポンスのキャッシュ
		- データ取得をサーバー上のみに限定
			- サーバー側でのみデータ取得関数が使用されるようにするため、 `server-only`パッケージの利用が推奨されている
				- 誤ってクライアントでサーバー関数を実行してしまうのを防げる
- 機密情報やオブジェクトインスタンス全体がクライアントに渡ってしまうのを防ぐために、Reactの `taint`APIの使用が推奨されている
	- 開発中にクライアント側への不適切なデータ送信を検出し、セキュリティリスクを未然に防ぐことが可能になる

#### Server Actions and Mutations
- Server Actionsとは、サーバー上で実行される非同期関数
	- Next.jsアプリケーションにおいて、フォームの送信やデータの変更を処理するために、Server ComponentおよびClient Componentの両方から呼び出すことができる
- Server ActionはReactの `use server`ディレクティブを使用して定義する
	- このディレクトリ内のを非同期関数の先頭に記述することで、その関数をServer Actionとしてマークできる
	- ファイルの先頭に記述することで、そのファイル内すべてのエクスポートをServer Actionとしてマークすることも可能
- Server ActionはClient ComponentにPropsとして渡すこともできる
- Server Actionは、 `<form>`要素の `action`属性を使って呼び出せる
	- Server Componentではデフォルトでプログレッシブ・エンハンスメントをサポートしており、JavaScriptがまだ読み込まれていない、あるいは無効化されている場合でもフォームの送信が可能
	- Client Componentでは、Server Actionを呼び出すフォームはJavaScriptが未読み込みの間は送信をキューに保持し、クライアントのハイドレーションが優先される
		- ハイドレーション後は、フォームの送信時にブラウザはリロードを行わない
- Server Actionは `<form>`に限定されず、イベントハンドラや `useEffect`, サードパーティーライブラリ、 `<button>`など他のフォーム要素からも呼び出せる
- Server Actionは内部的にPOSTメソッドを使用し、Server Actionを呼び出せるのはHTTPメソッドの中でPOSTだけ
- Server Actionの引数および戻り値は、Reactによってシリアライズ可能でなければならない
- ReactはHTMLの `<form>`要素を拡張し、 `action`プロパティでServer Actionを呼び出せるようにしている
