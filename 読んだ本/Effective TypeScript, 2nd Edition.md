...
### 進化する型を理解する
- TypeScriptの型は通常は絞り込まれるが、 `null`, `undefined`, `[]`で初期化された型は進化することが許されている
	- この仕組みがどこで発生するのかを理解し、自分のコードで型アノテーションの必要性を減らすために活用する
- より厳密なエラーチェックを行うために、進化する型を使うのではなく、明示的な型アノテーションを付けることを検討する

## 感想
- Effectiveがついている本なだけあって、型システムやTypeScript内部の仕組みに一歩踏み込んで解説してくれている本だと感じた
- 著者の豊富な経験から、「こういうケースではこうしよう」というのを具体的かつ明示的に示してくれている部分が多く、今後リファレンス的に読み返すのにも適していそう
- 型を値の集合として捉えるというのが個人的に新しい視点というか、型に対する腑に落ちる説明という感じ
	- TypeScript特有の仕組みである構造的部分型も説明しやすいと思う