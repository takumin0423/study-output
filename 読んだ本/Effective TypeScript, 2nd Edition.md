...
- 基本的に、TypeScriptは型を削除すればJavaScriptに変換できる
	- 列挙型、パラメータプロパティ、三重スラッシュインポート、実験的デコレーター、メンバーの可視性修飾子は、このルールの歴史的な例外
- TypeScriptの役割をコードベース内で明確にし、将来の互換性の問題を避けるために、非標準の機能は避ける

### TypeScriptのデバッグにはソースマップを使用する
- 生成されたJavaScriptをデバッグせず、ソースマップを使用してTypeScriptのコードを実行時にデバッグする
- ソースマップが実行するコードまで正しく対応していることを確認する
- TypeScriptで書かれたNode.jsのコードをデバッグする方法を理解しておく
- 設定によっては、ソースマップに元のコードのインラインコピーが含まれることがある
	- 意図しない限り公開しないように注意する

### 実行時に型を再構築する方法を理解する
- TypeScriptの型は、コードが実行される前に消去されるため、追加のツールなしでは実行時にアクセスできない
- 実行時の型の選択肢を理解する
	- 独自の実行時型システムを使用する
		- Zodなど
	- 値からTypeScriptの型を生成する
		- json-schema-to-typescriptなど
	- TypeScriptの型から値を生成する
		- typescript-json-schemaなど
- 別のスキーマなどで型を定義している場合、それを信頼できる情報源として使用する
- 外部のTypeScript型を参照する必要がある場合は、typescript-json-schema、または同等のツールを使用する
- それ以外の場合、ビルドステップを追加するか、型を指定する別のシステムを採用するか検討する

### DOM階層を理解する
- DOMには型の階層があり、JavaScriptを書く際には通常気にする必要はない
	- TypeScriptではこれらの型が重要になる
	- 理解することで、ブラウザ向けのTypeScriptを適切に記述できる
- `Node`, `Element`, `HTMLElement`, `EventTarget`の違い、および `Event`と `MouseEvent`の違いを理解する
- DOMの要素やイベントに対して十分に具体的な型を使用するか、TypeScriptが適切に推論できるようにコンテキストを提供する

### 環境を正確にモデル化する
- コードは特定の環境で実行される
	- 環境の静的モデルを正確に作成することで、TypeScriptはより適切にコードをチェックできる
- グローバル変数や、コードとともにWebページに読み込まれるライブラリをモデル化する
- 型定義のバージョンと、使用するライブラリやランタイム環境のバージョンを一致させる
- tsconfig.jsonを複数使用し、プロジェクトリファレンスを活用して、単一プロジェクト内で異なる環境をモデル化する
	- クライアントとサーバーなど

### 型チェックとユニットテストの関係を理解する
- 型チェックとユニットテストは、異なるが補完的な技術であり、プログラムの正しさを示すためにどちらも必要
- ユニットテストは特定の入力に対する正しい動作を示し、型チェックは特定の種類の誤った動作を根本的に排除する
- 型の検証は型チェッカーに任せ、型では検証できない動作についてはユニットテストを書く
- セキュリティやデータ破損の懸念がある場合を除き、型エラーとなるような入力をテストしないようにする

### コンパイラのパフォーマンスに注意を払う
- TypeScriptのパフォーマンス問題には、ビルドパフォーマンス（tsc）とエディタの遅延（tsserver）の2種類がある
	- それぞれの症状を理解し、適切に最適化する
- 型チェックをビルドプロセスとは別で行うようにする
- 不要なコードや依存関係を削除し、型の依存関係によるコードの肥大化に注意する
	- TypeScriptがコンパイルしている内容を可視化するためにツリーマップを活用する
- 増分ビルド（incrementalオプション）やプロジェクトリファレンスを利用して、tscのビルド間での処理を最小限に抑える
- 型を簡潔にする
	- 大きなユニオン型を避け、インターフェースの拡張を使用して交差型の使用を控え、関数の戻り値の型を明示的に注釈することを検討する

## 10. モダナイゼーションと移行
### 最新のJavaScriptを書く
- TypeScriptを使えば、実行環境に関係なく最新のJavaScriptが書ける
	- これを活用し、新しい言語機能を取り入れることで、コードベースを改善しつつTypeScriptによる解析を助ける
- TypeScriptへの移行をスムーズにするために、ESモジュール（`import`/ `export`）やクラスを採用する
- TypeScriptを活用して、クラス、分割代入、 `async`/ `await`などの言語機能を学ぶ
- 最新の言語機能について知るために、TC39のGitHubリポジトリやTypeScriptのリリースノートをチェックする

### @ts-checkとJSDocを活用してTypeScriptを試す
- JavaScriptファイルの先頭に@ts-checkを追加すると、TypeScriptに変換せずに型チェックを有効に出来る
- よくあるエラーを理解し、グローバル変数の宣言方法やサードパーティライブラリの型定義の追加方法を学ぶ
- JSDocアノテーションを活用して、型アサーションやより良い型推論を行う
- JSDocを使って完璧に型を定義することに時間をかけすぎないようにする
	- 最終的な目標は.tsファイルに移行すること

### allowJsを使用してTypeScriptとJavaScriptを混在させる
- allowJsコンパイラオプションを使用すると、プロジェクトの移行中にJavaScriptとTypeScriptを混在させることができる
- 大規模な移行を始める前に、テストやビルドチェーンがTypeScriptで正常に動作することを確認する

### 依存関係グラフを上流に向かってモジュールごと変換する
- 移行を開始する際は、サードパーティのモジュールや外部API呼び出しに対して@typesを追加する
- 依存関係グラフの下流から上流に向かって、自分のモジュールを順番にTypeScriptに移行する
	- 最初に変換するのは、ユーティリティコードなどの基盤となるモジュールであることが多い
	- 進捗を追跡するために、依存関係グラフを可視化するのも有効
- 移行中に奇妙な設計を見つけても、すぐにリファクタリングしないようにする
	- リファクタリングのアイデアをリスト化し、まずはTypeScriptへの変換に集中する
- 移行中によく発生するエラーを把握しておく
	- JSDocの型をTypeScriptの型アノテーションに移行することで、型安全性を維持しながら変換を進められる

### noImplicitAnyを有効にするまで移行が完了したと考えない
- noImplicitAnyを採用するまでは、TypeScriptへの移行が完了したとは考えないようにする
	- 緩い型チェックは、型定義の本当のミスを見逃す原因になる
- noImplicitAnyを強制する前に、型エラーを少しずつ修正していく
	- まずはチームがTypeScriptに慣れる時間を確保し、その後で厳格なチェックを導入するのが理想的