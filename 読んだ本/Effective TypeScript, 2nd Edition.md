...
- TypeScriptが提供する最も徹底したチェックを行うには strict を有効にする

### コード生成は型から独立していることを理解する
- tsc（TypeScriptコンパイラ）は以下の 2 つのことを行う
...
- 実行時に型情報を取得するための方法として、kindのようなプロパティを型に持たせ、それを用いて判別するタグ付き/判別ユニオン（tagged/discriminated unions）が TypeScriptでは広く使用されている
...
- 関数文は、型空間と値空間の間を繰り返し交互に切り替えることがある
...
- 各プリミティブに対応するオブジェクトラッパー型が定義されており、JavaScriptはこれらの型同士で自由に変換を行う
	- プリミティブに対してメソッドを呼び出すと、JavaScriptが対応するオブジェクトラッパーでプリミティブをラップしてメソッドを呼び出し、その後オブジェクトラッパーを破棄する
- プリミティブは対応するオブジェクトラッパー型に代入できるが、逆はできない
	- `string`は `String`に代入できるが、 `String`は `string`に代入できない
- **オブジェクトラッパー型ではなく、プリミティブ型を使用するべき**
	- TypeScriptに付随するすべての型宣言はプリミティブ型を使用しており、ほとんどのライブラリの型定義も同じようにプリミティブ型を使用している

### 余分なプロパティチェックと型チェックを区別する
- オブジェクトリテラルが宣言された型と一緒に使用される場合、未知のプロパティを許可しない
	- 「厳密なオブジェクトリテラルチェック」や「新規性」という意味で「フレッシュネス」と呼ばれる
		- 新しく作成されたオブジェクトに適用されるため
	- 中間変数を導入すると、この余分なプロパティチェックがなくなる
- TypeScriptにおける「弱い型」とは、オプショナルなプロパティのみを持つインターフェースを指す
	- 弱い型に対して、TypeScriptは **値の型と宣言された型に、少なくとも1つの共通のプロパティがあるかどうかをチェックする**
- 余分なプロパティチェックは、構造型システムで許可されるはずのプロパティ名のタイプミスやその他の間違いを検出する効果的な方法
	- オブジェクトリテラルのみに適用される